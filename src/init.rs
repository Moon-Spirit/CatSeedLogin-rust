...
pub async fn initialize_plugin(server: Arc<Context>) -> Result<(), String> {
    ...
    // 创建登录命令树
    let _login_tree = CommandTree::new(["login"], "Login command")
       .then(literal("login").execute(Login));
       
    // 创建注册命令树
    let _register_tree = CommandTree::new(["register"], "Register command")
       .then(literal("register").execute(Register));
       
    // 创建修改密码命令树
    let _changepassword_tree = CommandTree::new(["changepassword"], "Change password command")
       .then(literal("changepassword").execute(Changepassword));
       
    // 创建绑定邮箱命令树
    let _bindemail_tree = CommandTree::new(["bindemail"], "bindemail command")
       .then(literal("bindemail").execute(Bindemail));
       
    // 创建忘记密码重置命令树
    let _resetpassword_tree = CommandTree::new(["resetpassword"], "resetpassword command")
